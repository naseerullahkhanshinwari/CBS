  ...
  return car ? (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 mt-16'>
      <button onClick={() => navigate(-1)} className='flex items-center gap-2 mb-6 text-gray-500 cursor-pointer'>
        <img src={assets.arrow_icon} alt="backarrow" className='rotate-180 opacity-65' />
        Back to All Cars
      </button>
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12'>
          ...
          <div className='space-y-6'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-4'>
              {[
                ...
                { icon: assets.location_icon_icon, text: car.location },
              ].map(({ icon, text }) => (
                <div key={text} className='felx flex-col items-center bg-light p-4 rounded-lg'>
            ...
            {/*Features */}
            <h1 className='text-xl font-medium mb-3'>ٖFeatures</h1>
            <ul className='grid grid-cols-1 sm:grid-cols-2 gap2'>
              {
                ["Bluetooth", "GPS", "Heated Seats"].map((item) => (
                  <li key={item} className='items-center text-gray-500'>
        ...
        <form onSubmit={handleSubmit} className='shadow-lg h-max sticky top-18 rounded-xl p-6 space-y-6 text-gray-500'>
          ...
          <p className='text-center text-sm'>No Online Transaction</p>
        </form>
      </div>
    </div>
  ) : <Loader />
